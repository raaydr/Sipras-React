...
const handleDelete = async(event) => {
  ...
  try {
    const token = Cookies.get('tokenku')
    const response = await axios.delete(`/api/delete-barang/${idData}`,{ headers: {"Authorization" : `Bearer ${token}`} })
    setFetchStatus(true)
          ...
          <thead className="text-xs text-white uppercase bg-purple-500 dark:bg-purple-500 dark:text-white">
              ...
              <th scope="col" className={`px-6 py-3 ${products[0].show ? 'show' : 'hidden'}`}>
                  ...
                  {products.map((property,index) => {
                    ...
                    return (
                        <th scope="col" className={`px-6 py-4 ${property.show ? 'show' : 'hidden'} `}>
                            <input
                            type="text"
